...
def seen(phenny, input): 
   ...
   if phenny.seen.has_key(nick): 
      ...
      td = datetime.now() - datetime.fromtimestamp(t)
      timediff = [td.days, td.seconds // 3600, td.seconds // 60 % 60, td.seconds%60]
      t = time.strftime('%Y-%m-%d %H:%M:%S UTC', time.gmtime(t))

      #msg = "I last saw %s at %s on %s" % (nick, t, channel)
      msg = "I last saw %s %s days %s hours %s minutes %s seconds ago." % (nick, timediff[0], timediff[1], timediff[2], timediff[3])
      phenny.reply(msg)
